...
from .config import TransformerAutoencoderConfig


def build_transformer_autoencoder(token_dim: int, config: TransformerAutoencoderConfig | None = None):
    """Build a sequence autoencoder with one fixed-width latent vector per sample.

    The model consumes ``(batch, tokens, token_dim)`` tensors. It intentionally knows
    nothing about files, images, patchification, scalers, optimizers, or training loops.
    """
    if token_dim < 1:
        raise ValueError("token_dim must be positive")
    config = config or TransformerAutoencoderConfig()
    ...
    class TransformerAutoencoder(nn.Module):
        ...
        def encode(self, x: Any, *, element_mask: Any | None = None):
        ...
        def forward(self, x: Any, *, element_mask: Any | None = None):
            z = self.encode(x, element_mask=element_mask)
            return self.decode(z, token_count=int(x.shape[1]), element_mask=element_mask)
